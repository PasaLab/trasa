...
class Transformer(nn.Module):
    def __init__(self, num_layers, embed_dim, ff_embed_dim, num_heads, dropout, weights_dropout=True):
        super(Transformer, self).__init__()
        self.layers = nn.ModuleList()
        for _ in range(num_layers):
            self.layers.append(TransformerLayer(embed_dim, ff_embed_dim, num_heads, dropout, weights_dropout))
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, relation, kv=None, self_padding_mask=None, self_attn_mask=None, need_weights=False):
        residual = x
        if kv is None:
...
class RelationMultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, relation, key_padding_mask=None, attn_mask=None, need_weights=False):
        ...
        k = k.reshape(src_len, bsz * self.num_heads, self.head_dim)
        v = v.reshape(src_len, bsz * self.num_heads, self.head_dim)

        ra, rb = self.relation_in_proj(relation).chunk(2, dim=-1)
        # ra.size() = tgt_len * src_len * bsz * embed_dim
        ra = ra.reshape(tgt_len, src_len, bsz * self.num_heads, self.head_dim).transpose(0, 1)
        rb = rb.reshape(tgt_len, src_len, bsz * self.num_heads, self.head_dim).transpose(0, 1)
